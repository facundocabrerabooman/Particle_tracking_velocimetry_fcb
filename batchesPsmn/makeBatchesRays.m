...
BATCHPATH=[HOME filesep 'LEM/Sander/TracersHIT/batchesRays'];
WORKDIR=[HOME filesep 'LEM/PTV_LEM'];
QUEUE='monointeldeb48';
EXECFILE=[HOME filesep 'LEM/PTV_LEM/run_centers2rays.sh'];
...
[status,cmdout]=unix(['find ' RESULTSFOLDER ' -name "centers_*.dat" -not -path "*alibra*"']);
...
folderout=cellfun(@(X)([RESULTSFOLDER filesep char(strjoin(X(2:end-1),'_'))]),prefix,'UniformOutput',false);

% generate prefix
prefix=cellfun(@(X)([char(strjoin(X(2:end-1),'_')) '_']),prefix,'UniformOutput',false);
...
for k=1:numel(folder)
    ...
    fid = fopen(batchname,'w');
        ...
        fprintf(fid,'%s %s %s %s %s %s %s %s\n',EXECFILE,RUNTIMEFOLDER,...
            folder{k},folderout{k},CALIBFILE,prefix{k},Nmes{k},Ncam);  
    fclose(fid);
end
