...
BATCHPATH='/home/mbourgoi/LEM/Sander/TracersHIT/batches';
WORKDIR='/home/mbourgoi/LEM/PTV_LEM';
QUEUE='monointeldeb48';
EXECFILE='/home/mbourgoi/LEM/PTV_LEM/run_CenterFinding2D.sh';
...
prefix=cellfun(@(X)(strsplit(char(X),'.mcin2')),fname,'UniformOutput',false);
prefix=cellfun(@(X)(strsplit(char(X(1)),'_cam')),prefix,'UniformOutput',false);%
...
folderout=cellfun(@(X)([RESULTSFOLDER filesep char(strjoin(X(1:end-1),'_'))]),prefix,'UniformOutput',false);
...
for k=1:numel(folder)
    ...
    fid = fopen(batchname,'w');
        ...
        fprintf(fid,'%s %s %s %s %s %s %s %s %d %d\n',EXECFILE,RUNTIMEFOLDER,...
            folder{k},folderout{k},prefix{k},Nmes{k},NmesNcamSep,Ncam{k},nstart,nframes);  
    fclose(fid);
end
