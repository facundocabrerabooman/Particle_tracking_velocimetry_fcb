function CC = CenterFinding2Dc(folderin,folderout,fname,cam, flag_video, flag_plot)
%%
...
if images_format == 'DNG'
    ...
    tic
    files = dir([folderin filesep '*.JPG' ]); toc
end
nframes = numel(files);
fprintf(fid,'%d\n',nframes);
...
if video_flag==1
    %%video
    video = VideoWriter([fname cam '.avi']); %create the video object
    video.FrameRate = 1400;
    open(video);
end
%%%
index_no_particles_found = 0;
for kframe=1:nframes %START IN 3 WHEN YOU HAVE A WAKE, ALSO CHANGE IMPORT IM0
    ...
    if ~isempty(PP.Centroid)
        if ~(PP.Centroid == [640.5 360.5]) %delete phantoms. When there isn't particle detects a phantom in center of image
            kframe = kframe - index_no_particles_found ;
            %
            cc=[PP.Centroid];
            ...
            if kframe/10 == round(kframe/10)     %plot each 40 steps
                imshow(im), hold on
                plot(x,y,'ro');title(num2str(kframe)), pause(0.5)
...
if video_flag==1 close(video) end
%
% % %% Track
