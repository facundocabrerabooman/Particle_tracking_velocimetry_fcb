function CC = CenterFinding2Dc_savesphantoms(folderin,folderout, fname, cam, plot_flag, video_flag)
%%
...
if ~isempty(dir([folderin filesep '*.DNG']))
    ...
    %%%
    for kframe=2:nframes %START IN 3 WHEN YOU HAVE A WAKE, ALSO CHANGE IMPORT IM0
        kframe/(nframes)
        kframe
        ...
        im0 = imread([folderin filesep files(1).name]); %WHEN NO WAKE
       ...
       %im=imsubtract(imcomplement(im),imcomplement(im0)); % remove background (im0) of im
        im=imcomplement(im); % don't remove background (im0) of im
        im = imbinarize(im);
        ...
        PP=regionprops(Bw,'Centroid','Area','MajorAxisLength','MinorAxisLength','Orientation');
        %%
        PP.Centroid
       %% atention when chain
                %cc=[PP.Centroid];      %original                
                cc=vertcat(PP.Centroid);    % to take vertical from chain - keeps all centers found
                %%
                
%                 Ma=[PP.MajorAxisLength]; %ORIGINAL
%                 ma=[PP.MinorAxisLength];
%                 O = [PP.Orientation];
%                 A = [PP.Area];

                Ma=vertcat(PP.MajorAxisLength); %changed for analyse chain = more than one part
                ma=vertcat(PP.MinorAxisLength);
                O = vertcat(PP.Orientation);
                A = vertcat(PP.Area);
                x=cc(:,1);
                y=cc(:,2);
                ...
                CC(kframe).A=A';
                %% checking plots
              %  if ne(PP.Centroid(1),[640.5000 360.5000])
                if plot_flag == 1 
                    %if kframe/10 == round(kframe/10)      %plot each 40 steps
                       clf, imshow(Bw), hold on
                        plot(cc(:,1),cc(:,2),'r.');title(num2str(kframe)), pause(0.5)
